...
class Mwb_Bookings_For_Woocommerce_Common {
	...
	public function mbfw_retrieve_booking_total_single_page() {
		...
		$wedding_price = get_post_meta( $product_id, 'wedding', true );
		$price_type = get_post_meta( $product_id, 'price_type', true );

		$product_price = (float) $product_price * (float) $unit;

		if($price_type == 'percentage') {
			$per = $product_price * ($wedding_price / 100);
			if($wedding_price && !empty($wedding_price)) {
				if($unit > 2) {
					$product_price = $product_price - $per;
				}
			}
		}
		...
		if($price_type == 'fixed') {
			if($wedding_price && !empty($wedding_price)) {
				if($unit > 2) {
					$product_price = $unit * $wedding_price;
				}
			}
		}
		...
		<div class="mbfw-total-listing-single-page__wrapper-parent">
			...
			foreach ( $charges as $types ) {
				...
				<div class="mbfw-total-listing-single-page__wrapper">
					<div class="mbfw-total-listing-single-page">
						<?php
						if ( 'Service Cost' != $title && 'Base Cost' != $title ) {
					...
					<div class="mbfw-total-listing-single-page">
						<?php if ( 'Service Cost' != $title && 'Base Cost' != $title ) {
